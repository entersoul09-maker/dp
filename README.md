...
<script>
    ...
    function renderPalette() {
        ...
        pList.innerHTML = paletteData.map(name => {
            const isSelected = selectedColors.has(name);
            return `<div class="palette-btn ${isSelected?'selected':''}" onclick="toggleColor(this, '${name}')">${name}</div>`;
        }).join('');
    }
    ...
    function autoCalc() {
        let dateInput = document.getElementById('arrivalDate').value;
        if (!dateInput) return;
        let date = new Date(dateInput);
        let workDayCount = (date.getDay() !== 0 && date.getDay() !== 6) ? 1 : 0;
        while (workDayCount < 6) {
            date.setDate(date.getDate() + 1);
            if (date.getDay() !== 0 && date.getDay() !== 6) workDayCount++;
        }
